...
namespace operations_research {
    ...
    std::iota(std::begin(days), std::end(days), 0);
    {
      ...
      LOG(INFO) << "Days:" << oss.str();
		...
		std::array<std::array<IntVar *, nurses.size()>, days.size()> shifts_matrix;
		...
		for (const auto day :days) {
			...
			for (const auto nurse: nurses) {
				solver.AddConstraint(
						solver.MakeIndexOfConstraint(nurses_for_day, shifts_matrix[nurse][day], nurse));
			}
		}
		...
		std::array<std::array<IntVar *, shifts.size()>, nurses.size()> works_shift_matrix;
		for (const auto nurse: nurses) {
			for (const auto shift: shifts) {
				std::ostringstream oss;
				oss << "work_shift(nurse: " << nurse << ", shift: " << shift << ")";
				IntVar *var = solver.MakeBoolVar(oss.str());
				works_shift_matrix[nurse][shift] = var;
			}
		}
		...
		for (const auto nurse: nurses) {
			for (const auto shift: shifts) {
				...
				for (const auto day: days) {
					// Todo HowTo write it ?
					//if (shifts_matrix[nurse][day] == shift) shift_s_for_nurse.push_back(shifts_matrix[nurse][day]);
					shift_s_for_nurse.push_back(solver.MakeIsEqualCstVar(shifts_matrix[nurse][day], shift));
				}
				solver.AddConstraint(
						solver.MakeEquality(
							works_shift_matrix[nurse][shift],
							solver.MakeMax(shift_s_for_nurse)));
			}
		}
		...
		for (std::size_t shift=1; shift < shifts.size(); ++shift) {
			...
			for (const auto nurse: nurses) {
				...
				solver.AddConstraint(
					solver.MakeSumLessOrEqual(nurses_for_shift, 2));
		}
